		...
		for (const auto& input_info : map_info.second)
		{
			...
			case InputType::KEYBOARD:
				...
				case Axis::KEY_ARROW_X:
					state = -(direct_input_->GetKey(Button::KEY_LEFT) & 1) + (direct_input_->GetKey(Button::KEY_RIGHT) & 1);
					break;
				case Axis::KEY_ARROW_Y:
					state = (direct_input_->GetKey(Button::KEY_UP) & 1) - (direct_input_->GetKey(Button::KEY_DOWN) & 1);
					break;
				case Axis::KEY_WASD_X:
					state = -(direct_input_->GetKey(Button::KEY_A) & 1) + (direct_input_->GetKey(Button::KEY_D) & 1);
					break;
				case Axis::KEY_WASD_Y:
					state = (direct_input_->GetKey(Button::KEY_W) & 1) - (direct_input_->GetKey(Button::KEY_S) & 1);
					break;
				}
				break;
