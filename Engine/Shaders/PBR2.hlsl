...
float4 main(VSOutput input) : SV_TARGET
{
    ...
    if (possm.x > 0 && possm.x < 1 && possm.y > 0 && possm.y < 1)
    {
        ...
        //shadow = PoissonShadow(possm, 10, 0.5, uv);
        //shadow = Shadow(possm);
    }
    ...
    //float3 irradiance = gSpecularMap.SampleLevel(gSampler, N, 9).rgb;
    ...
    float3 ambient = (kD * diffuse + specular) * ao * lerp(0.5, 1, Lo * shadowColor);
