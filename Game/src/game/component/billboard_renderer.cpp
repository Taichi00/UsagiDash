...
BillboardRenderer::BillboardRenderer(std::shared_ptr<Texture2D> albedoTexture, std::shared_ptr<Texture2D> normalTexture)
{
	...
	float width2 = width * 0.005f;
	float height2 = height * 0.005f;
	width_ = width2;
	height_ = height2;
	...
	material.base_color = { 0.72f, 0.72f, 0.72f, 1.0f };
...
void BillboardRenderer::UpdateCB()
{
	...
	auto ditherLevel = (1.0f - ((transform->position - camera->transform->position).Length() - 3) / 3.0f) * 16;
