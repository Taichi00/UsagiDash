...
bool CollisionTestScene::Init()
{
	...
	player->AddComponent(new Player(20, 1.2f));

	player->GetComponent<Animator>()->Play("Idle", 2.0f);
	player->GetComponent<Collider>()->offset = Vec3(0, 1.5f, 0);
	...
	test_label->AddComponent(new Label("<bitmap input_jump>‚ÅƒWƒƒƒ“ƒv", text_prop, Color(0.35f, 0.3f, 0.3f)));
